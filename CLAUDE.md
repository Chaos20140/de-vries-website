...
- **Mobile-Nav**: `#navBurger` öffnet `#mobileNav` (clip-path-circle reveal, per-Element `--i`-Stagger), sperrt Body-Scroll + `lenis.stop()`. Esc/Close/Linkklick schließt. **Struktur (redesignt, ruhig & geordnet):** Kopf `.mobile-nav__top` (echtes Logo `.mobile-nav__brand` + runder `.mobile-nav__close`) → `.mobile-nav__body` mit roten Gruppen-Eyebrows `.mnav__label` (LEISTUNGEN / MEHR) und Zeilen-Links `.mnav__link` (Fraunces, **KEINE** Trennlinien/Chevrons mehr — Ruhe durch Weißraum; aktive Seite rot via `[aria-current]`, Hover `translateX`) → Footer `.mobile-nav__foot` mit `.mnav__cta` (roter „Termin buchen"-Button) + `.mnav__contact` (**nur Mail + Adresse mit Icons — Telefon bewusst entfernt**, Nummer bleibt in Topbar/Footer). **Seniorenbetreuung = Accordion:** `<button.mnav__toggle aria-controls="mnavSenioren" aria-expanded>` mit Chevron (`.mnav__chev` dreht 180° bei `aria-expanded=true`); `.mnav__sub` ist standardmäßig zugeklappt (`max-height:0; opacity:0`) und öffnet via `.is-open` (max-height-Transition). Unterpunkte: Übersicht (→seniorenbetreuung.html) · Entlastungsbetrag · Pflegesachleistungen. **Startet IMMER eingeklappt** (auch auf Unterseiten — kein Auto-Aufklappen, vom Nutzer so gewünscht). **Wichtig:** `.mobile-nav__body` braucht `overflow: hidden auto` (nur `overflow-y:auto` macht `overflow-x` implizit zu `auto` → Geister-Horizontalbalken); `.mnav__sub` NICHT im Stagger (Accordion steuert es). JS-Selektoren entkoppelt: Close-on-Click = `$$("a[href]", mobileNav)` (Toggle ist `<button>`, schließt NICHT), Active-Link = `.nav__links a, #mobileNav a[href]`, Accordion-Toggle = `.mnav__toggle`.
- **Cookie**: `#cookie` nach 1.4s ein, `localStorage 'dv-cookie-ok'` merkt Zustimmung.
- **Kontaktformular**: `#contactForm` → Validierung (Name/E-Mail/Nachricht/Consent) → **`mailto:`-Fallback**. Kein Backend.
- **⚠️ Formular-Empfänger TEMPORÄR umgeleitet**: beide Formular-`mailto:` (Kontakt + Termin, in `main.js`) gehen aktuell an **`tolunayusul@gmail.com`** (Test/Übergang, vom Nutzer gewünscht), NICHT an info@andreasdevries.de. Die ANGEZEIGTEN Kontaktdaten (Topbar/Footer/Info-Card/Impressum) bleiben `info@andreasdevries.de`. Bei „mails sollen wieder an de Vries": die 2 `mailto:tolunayusul@gmail.com` in `main.js` zurück auf `info@andreasdevries.de` (Nutzer schrieb erst „gmail.de" → Tippfehler, korrekt ist .com).
- **Echtes Buchungs-Backend (Supabase) — optional, via Config aktiviert**: Code liegt unter `supabase/` (Edge Function `devries-booking` + Migration `devries_bookings`), Frontend-Config in `assets/js/booking-config.js`. Solange dort `DEIN-…`-Platzhalter stehen → `backendOn=false` → Termin-Formular nutzt **mailto-Fallback** (Site funktioniert ohne Backend). Sind echte `url`+`anonKey` eingetragen: Termin-Submit → `POST /booking` (Status pending, mailt Inhaber Bestätigen/Ablehnen-Links mit geheimem Token), Kalender lädt `GET /booked-slots` und sperrt **bestätigte** Uhrzeiten (`.slot.is-taken`/`disabled`). Sperre NUR pro Uhrzeit, NUR nach Bestätigung (so vom Nutzer gewählt). Einrichtung: `SETUP-BUCHUNG.md`. Backend kann ich nicht selbst testen (Nutzer-Credentials nötig). Wichtig: `booking-config.js` muss in termin.html VOR `main.js` geladen werden; Cache-Version (`?v=N`) bei Änderung hochzählen.
...
- **⚠️ `clamp()`/`calc()` brauchen Leerzeichen um `+` und `-`!** `clamp(2rem,1.4rem+3vw,4rem)` ist UNGÜLTIG (CSS verwirft die ganze Eigenschaft → Wert fällt auf 0/initial). RICHTIG: `clamp(2rem, 1.4rem + 3vw, 4rem)`. Das war der Root-Cause der „alles gequetscht"-Beschwerden: ~33 Komponenten-Clamps (Karten-Padding, Section-Head-Margin, Feature-/Galerie-Gaps) waren tot → 0 Abstand. Check: `grep -oE 'clamp\([^)]*\)' style.css | grep -E '[a-z0-9]\+[0-9.]'` muss LEER sein. (`*` und `/` brauchen keine Spaces, nur `+`/`-`.)
