...
- **Mobile-Nav**: `#navBurger` öffnet `#mobileNav` (clip-path-circle reveal, per-Element `--i`-Stagger), sperrt Body-Scroll + `lenis.stop()`. Esc/Close/Linkklick schließt. **Struktur (redesignt, geordnet):** Kopf `.mobile-nav__top` (echtes Logo `.mobile-nav__brand` + runder `.mobile-nav__close`) → scrollbarer `.mobile-nav__body` mit roten Gruppen-Eyebrows `.mnav__label` (LEISTUNGEN / MEHR) und Zeilen-Links `.mnav__link` (Fraunces, Trennlinie + Chevron-`::after`, aktive Seite rot via `[aria-current]`), Unterpunkte in `.mnav__sub` (linker Border + rotes ›) → Footer `.mobile-nav__foot` mit `.mnav__cta` (roter „Termin buchen"-Button) + `.mnav__contact` (Tel/Mail/Adresse mit Icons). **Wichtig:** `.mobile-nav__body` braucht `overflow: hidden auto` (nur `overflow-y:auto` macht `overflow-x` implizit zu `auto` → Geister-Horizontalbalken). JS-Selektoren entkoppelt: Close-on-Click = `$$("a[href]", mobileNav)`, Active-Link = `.nav__links a, #mobileNav a[href]` (NICHT mehr `.mobile-nav__links`).
